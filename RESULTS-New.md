...
Imagine though you are the front desk of a medical practice office, admitting patients.

This few seconds adds up quickly, particularly when there are multiple folks queued up.
...
The `--where-'$dur<1` option is frequently used to filter out calls lasting 1 second or more, which in many cases consists of `SQL*Net message from client calls'.
...
While most of the time a user may require more than 1 second between operations that result in the app communicating with the database, there may be occasions where the use can do this in less than 1 second.
...
Nonetheless, we will use 0.1 second as the time to exclude.

It is concievable that other calls could exceed 1 second as well.  It is not unusual to see any of PARSE,EXEC or FETCH exceed that time.

So, let's get an idea of max time per call, so that exclusion of calls we do want to influence the output are at least mimimized.

The following summarizes all database calls and system calls.
...
```text
$  time mrskew --name='FETCH' --rc=p10.rc  trace-2-methodr.trc trace*/*.trc
RANGE {min ≤ e < max}            DURATION       %      CALLS      MEAN       MIN       MAX
-----------------------------  ----------  ------  ---------  --------  --------  --------
...
The calls GE 0.1 seconds make up ~ 31% of all FETCH time, which is a significant amount.
...
If all FETCH calls GE .7 second are not included in the results, the difference in measurement will be minimal.
...
Again, even though 27.37 seconds of PARSE time will not be included in the report for all trace files when cpu time is limited to 0.7 seconds, the amount of time will not be significant in the report.


Now let's get an overview of where this app spends its time, at least from a database perspective.
...
This app spends about 88% of it's time waiting on SQL\*Net traffic.  This is rather a lot of time.
...
The term not the practice; he was dead-set against row-by-row processing.

You may have experienced this by experimenting with the 'arraysize' setting in sqlplus.

If not, here's a simple test that will show the significance of this concept.
...
And that is with the client located physically next to the database.

We can use `mrskew` to find out just how many rows are returned at a time.
...
So now the question is this:  Are those 1 row packets being sent because application has submitted a SQL query that is expected to return only 1 row?

Or is it due the app being configured to retrieve one row at a time, regardless of the number of rows that may be returned?
...
It really depends on which part of the app the user is running.

One group of users experience 30% of SNMFC occurring from this same query, so those folks are likely to notice any improvement.
...
Not having used CRC before this time, some reading was necessary.  That task did not take much time, as there is little documenation, and scarcely anything written on the topic.

The premise is to dedicate a few megabytes of memory at each client for caching SQL results.

Subsequent queries may be satisified from the cache, rather than making a SQL\*Net rount trip.
...
In general, CRC is fairly easy to enable:
...
The following Bash script is used as driver:
...
CHIAR refers to Dental chairs, which will change infrequently.
...
The testing will also use the `mrskew` option of `'--where-'$dur<1'` as it was against the trace data from the application.
